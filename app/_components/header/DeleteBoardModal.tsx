export default function DeleteBoardModal({
  ...
  return (
    <div className="absolute left-0 top-0 z-10 h-screen w-screen select-none">
      <div className="bg-black/40 absolute left-0 top-0 h-full w-full"></div>
      <div
      ...
      >
        ...
        <p className="mb-[22px] pr-1 text-[13px] leading-[23px] text-neutral-500">
          Are you sure you want to delete the ‘{currentBoard.name}’ board? This
          action will remove all columns and tasks and cannot be reversed.
        </p>
        <div className="flex flex-col gap-4">
          <button
            className={`h-[40px] cursor-pointer rounded-[20px] bg-warning text-[13px] font-semibold leading-[13px] text-neutral-800`}
            onClick={() => handleRemoveBoard(currentBoard.id)}
          >
            Delete
          </button>
          <button
            className="h-[40px] cursor-pointer rounded-[20px] bg-neutral-800 text-[13px] font-semibold leading-[13px] text-primary"
            onClick={(e) => {
